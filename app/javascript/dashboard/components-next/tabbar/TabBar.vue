...
<template>
  <div
    class="relative flex items-center h-8 rounded-lg bg-n-alpha-1 dark:bg-n-solid-1 w-fit transition-transform duration-200 ease-out has-[button:active]:scale-[1.01]"
  >
    <div
      ...
      :class="{ 'transition-[left,width] duration-300 ease-out': enableTransition }"
    ...
    <template v-for="(tab, index) in tabs" :key="index">
      <button
        ...
        class="relative z-10 px-4 truncate py-1.5 text-sm border-0 outline-1 outline-transparent rounded-lg transition-[color,transform] duration-200 ease-out hover:text-n-brand active:scale-[1.02]"
        :class="[
          activeTab === index
            ? 'text-n-slate-12 font-medium scale-100'
