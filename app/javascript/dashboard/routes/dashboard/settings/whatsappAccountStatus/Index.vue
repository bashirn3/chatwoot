...
const channels = computed(
  () => store.getters['whatsappAccountStatus/getChannels']
);
const summary = computed(
  () => store.getters['whatsappAccountStatus/getSummary']
);
const alerts = computed(() => store.getters['whatsappAccountStatus/getAlerts']);
const uiFlags = computed(
  () => store.getters['whatsappAccountStatus/getUIFlags']
);
  ...
  for (const channel of channels.value) {
    try {
      await store.dispatch(
        'whatsappAccountStatus/syncChannelStatus',
        channel.inbox_id
      );
    } catch (error) {
...
<template>
  <div class="flex-1 overflow-auto p-6">
    ...
    <div class="grid grid-cols-4 gap-4 mb-6">
      <div class="bg-white rounded-xl border border-slate-200 p-4">
        <p class="text-sm text-slate-500 mb-1">
          {{ $t('WHATSAPP_ACCOUNT_STATUS.SUMMARY.TOTAL') }}
        </p>
        <p class="text-2xl font-bold">{{ summary.total }}</p>
      </div>
      <div class="bg-white rounded-xl border border-slate-200 p-4">
        <p class="text-sm text-slate-500 mb-1">
          {{ $t('WHATSAPP_ACCOUNT_STATUS.SUMMARY.ACTIVE') }}
        </p>
        <p class="text-2xl font-bold text-green-600">{{ summary.active }}</p>
      </div>
      <div class="bg-white rounded-xl border border-slate-200 p-4">
        <p class="text-sm text-slate-500 mb-1">
          {{ $t('WHATSAPP_ACCOUNT_STATUS.SUMMARY.WITH_ISSUES') }}
        </p>
        <p class="text-2xl font-bold text-red-600">{{ summary.with_issues }}</p>
      </div>
      <div class="bg-white rounded-xl border border-slate-200 p-4">
        <p class="text-sm text-slate-500 mb-1">
          {{ $t('WHATSAPP_ACCOUNT_STATUS.SUMMARY.LOW_QUALITY') }}
        </p>
      ...
      <div class="bg-red-50 border border-red-200 rounded-xl p-4">
        <div class="flex items-center gap-2 mb-3">
          ...
          <h3 class="font-semibold text-red-800">
            {{ $t('WHATSAPP_ACCOUNT_STATUS.ALERTS.TITLE') }}
          </h3>
        </div>
        <div class="space-y-2">
          <div
    ...
    >
      <svg
        class="w-12 h-12 mb-4 text-slate-400"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
      ...
      <h3 class="text-lg font-medium mb-2">
        {{ $t('WHATSAPP_ACCOUNT_STATUS.NO_CHANNELS') }}
      </h3>
    ...
    <div v-else class="space-y-4">
      <div
        ...
        class="bg-white rounded-xl border p-5 transition-all"
        :class="[
      ...
      >
        <!-- BANNED Banner -->
        <div
          v-if="channel.account_status === 'BANNED'"
          class="mb-4 p-4 bg-red-100 border-2 border-red-500 rounded-lg"
        >
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-sm text-red-700">
                This WhatsApp Business Account has been banned by Meta. Please
                review Meta's policies and submit an appeal through the Meta
        ...
        <!-- RESTRICTED Banner -->
        <div
          v-else-if="channel.account_status === 'RESTRICTED'"
          class="mb-4 p-4 bg-orange-100 border-2 border-orange-500 rounded-lg"
        >
          <div class="flex items-center gap-3">
            <svg
            ...
            <div>
              <h4 class="font-bold text-orange-800 text-lg">
                Account Restricted
              </h4>
              <p class="text-sm text-orange-700">
                This account has restrictions applied. Some features may be
                limited. Check Meta Business Suite for details.
              </p>
            </div>
          </div>
        </div>
        ...
        <!-- FLAGGED Banner -->
        <div
          v-else-if="channel.account_status === 'FLAGGED'"
          class="mb-4 p-4 bg-yellow-100 border-2 border-yellow-500 rounded-lg"
        >
          <div class="flex items-center gap-3">
            <svg
            ...
            <div>
              ...
              <p class="text-sm text-yellow-700">
                This account is under review by Meta. Continue following best
...
:class="[
              :class="
[getStatusColor(channel.account_status)]"
            >
            ...
            <span
              ...
              :class="[getQualityColor(channel.quality_rating)]"
            >
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-4">
          <div>
            <p class="text-xs text-slate-500 uppercase">
              {{ $t('WHATSAPP_ACCOUNT_STATUS.DETAILS.MESSAGING_LIMIT') }}
            </p>
            <p class="font-medium">
              {{
                formatTier(
                  channel.messaging_limit_tier,
                  channel.messaging_limit_number
                )
              }}
            </p>
          </div>
          <div>
            ...
            <p class="font-medium text-lg">
              {{ formatMessageCount(channel.messages_sent_24h) }}
            </p>
          </div>
          <div>
            <p class="text-xs text-slate-500 uppercase">
            ...
            <p class="font-medium">
              {{ formatMessageCount(channel.messages_sent_total) }}
            </p>
          </div>
          <div>
            <p class="text-xs text-slate-500 uppercase">
              {{ $t('WHATSAPP_ACCOUNT_STATUS.DETAILS.THROUGHPUT') }}
            </p>
            <p class="font-medium">
              {{
                channel.current_throughput
                  ? `${channel.current_throughput} mps`
                  : 'Standard'
              }}
            </p>
          </div>
          <div>
            <p class="text-xs text-slate-500 uppercase">
              {{ $t('WHATSAPP_ACCOUNT_STATUS.DETAILS.BUSINESS_VERIFICATION') }}
            </p>
            <p class="font-medium">
              {{ channel.business_verification_status || 'Pending' }}
            </p>
          </div>
          <div>
            <p class="text-xs text-slate-500 uppercase">
              {{ $t('WHATSAPP_ACCOUNT_STATUS.LAST_SYNCED') }}
            </p>
            <p class="font-medium text-sm">
              {{ formatDate(channel.last_synced_at) }}
            </p>
          </div>
        </div>
        ...
        <div
          v-if="
            channel.violations && Object.keys(channel.violations).length > 0
          "
          class="mb-4"
        >
          <p class="text-xs text-red-600 uppercase font-semibold mb-2">
            {{ $t('WHATSAPP_ACCOUNT_STATUS.DETAILS.VIOLATIONS') }}
          </p>
        ...
        <div
          v-if="channel.restrictions && channel.restrictions.length > 0"
          class="mb-4"
        >
          <p class="text-xs text-orange-600 uppercase font-semibold mb-2">
            {{ $t('WHATSAPP_ACCOUNT_STATUS.DETAILS.RESTRICTIONS') }}
          </p>
          <div class="flex flex-wrap gap-2">
            <span
