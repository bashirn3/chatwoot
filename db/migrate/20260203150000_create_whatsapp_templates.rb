...
class CreateWhatsappTemplates < ActiveRecord::Migration[7.0]
  def change
    # Only create if table doesn't exist (safe for Supabase with existing data)
    return if table_exists?(:whatsapp_templates)
    ...
    add_index :whatsapp_templates, [:account_id, :name, :language], unique: true, name: 'idx_whatsapp_templates_unique_name' unless index_exists?(:whatsapp_templates, [:account_id, :name, :language])
    
    # Index for organization-based queries (Clerk)
    add_index :whatsapp_templates, [:clerk_organization_id, :status], name: 'idx_whatsapp_templates_org_status' unless index_exists?(:whatsapp_templates, [:clerk_organization_id, :status])
    
    # Index for finding templates by status for sync jobs
    add_index :whatsapp_templates, [:status, :last_synced_at], name: 'idx_whatsapp_templates_sync' unless index_exists?(:whatsapp_templates, [:status, :last_synced_at])
  end
end
